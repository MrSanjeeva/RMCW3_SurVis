define([
  ...
  "app/util",
  "data/generated/bib",
  "data/generated/available_pdf",
  "data/generated/available_img",
  "data/search_stopwords",
  "data/tag_categories",
  "data/authorized_tags",
], function (
  $,
  bibtexJS,
  fileSaver,
    ...
    addEntries: function () {
      ...
      bibtexEditor.on("change", function (bibtexEditor) {
        ...
        try {
          ...
          $("<div>", {
            html:
              Object.keys(bibtexEntries).length === 0
                ? "No entries"
                : util.objectToString(bibtexEntries),
            class: "info",
              ...
              for (var entryKey in bibtexEntries) {
                ...
                if (bib.entries[entryKey]) {
                  alert(
                    'Entry with ID "' +
                      entryKey +
                      '" already exists and cannot be added to the database.'
                  );
                  continue;
                }
  ...
  function readBibtex() {
    ...
    var loadFromLocalStorage =
      util.getUrlParameter("loadFromLocalStorage") === "true";
    if (editable && loadFromLocalStorage && localStorage.bibtexString) {
      try {
        ...
        console.error(err);
        alert(
          "Could not load bibliography from local storage, loaded default instead: \n" +
            err.substring(0, 200)
        );
      }
      return null;
